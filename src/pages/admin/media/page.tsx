...
type TabType = 'all' | 'property' | 'banner';
...
const AdminMedia = () => {
  ...
  const handleDelete = async (img: MediaImage) => {
    ...
    setDeleting(img.id);
    await supabase.storage.from('media').remove([img.file_path || img.url]);
    ...
    <AdminLayout>
      <div className="p-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-stone-800">Hình ảnh & Banner</h1>
            <p className="text-stone-500 text-sm mt-1">Upload và quản lý ảnh căn hộ, banner trang chủ</p>
          </div>
          <button
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-1 bg-stone-100 rounded-lg p-1">
            {([['all', 'Tất cả'], ['property', 'Ảnh căn hộ'], ['banner', 'Banner']] as [TabType, string][]).map(([val, label]) => (
              <button
                key={val}
                onClick={() => setTab(val)}
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${
                  tab === val ? 'bg-white text-stone-800' : 'text-stone-500 hover:text-stone-700'
                }`}
              >
                {label}
              </button>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-stone-100 py-16 text-center">
            ...
            <p className="text-stone-400 text-xs mt-1">Nhấn "Upload ảnh" để thêm hình ảnh đầu tiên</p>
          </div>
        ) : (
        ...
        {!loading && (
          <div className="mt-5 text-center text-stone-400 text-xs">
            Hiển thị {filtered.length} / {images.length} ảnh
          </div>
        )}
      </div>
          ...
          >
            <div className="relative">
              <img
                ...
                className="w-full max-h-72 object-cover object-top rounded-t-2xl"
              />
              <button
                onClick={() => setSelectedImage(null)}
              ...
              {selectedImage.is_banner && (
                <div className="absolute top-3 left-3 bg-amber-500 text-white text-xs px-2 py-1 rounded-lg font-medium">
                  Banner trang chủ
                </div>
              )}
            </div>
            <div className="p-5">
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 border border-stone-200 rounded-lg px-3 py-2 text-xs text-stone-500 bg-stone-50"
                  />
                  <button
                    onClick={() => handleCopyUrl(selectedImage.url, selectedImage.id)}
                    className="px-3 py-2 bg-stone-100 hover:bg-amber-50 text-stone-600 hover:text-amber-700 rounded-lg text-xs font-medium transition-colors cursor-pointer whitespace-nowrap flex items-center gap-1.5"
                  >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-colors cursor-pointer whitespace-nowrap ${
                    selectedImage.is_banner
